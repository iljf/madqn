...
def main():
    ...
    for ep in range(args.total_ep):
        ...
        for agent in env.agent_iter():
            ...
            # 매 스텝이 끝나고 predator1 돌기 바로 전에 돌아가는 과정
            ...
            if (((iteration_number) % (args.n_predator1 + args.n_predator2 + args.n_prey)) == 0) and (iteration_number > 0):
                ...
                if step_idx != args.max_update_steps :
                    ...
                    # 마지막 스텝이 아닌(== truncation 하지 않는 부분에서 작동한느 코드)
                    ...
                    # madqn.summation_team_dist : step별 팀별 prey까지의 거리들을 저장한 list
            ...
            if agent[:8] == "predator":
                ...
                if agent[9] == "1":
                    ...
                    # 아래와 같이 평균내어 저장하는것이 좀 더 올바른 해석방법인 것 같다.
                    dist_list = np.array(np.mean(madqn.dist(observation_temp)),dtype=float)
                    #np.array([np.array(np.mean(dist_list))], dtype=float)
                ...
                if termination or truncation:
                    ...
                    env.step(None)
                    #truncation 해서 끊길때도 iteration_number는 반드시 해주어야 한다.
                    iteration_number += 1
                    continue
