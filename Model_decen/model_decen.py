...
class G_DQN_1(nn.Module):
    def __init__(self,  dim_act, observation_state, args):
        ...
        #DQN
        self.dim_input = ((args.predator1_view_range *2)**2)*self.dim_feature
    ...
    def forward(self, x, adj):
        ...
        if x.dim() == 3:
            x_b = x.reshape(num_nodes, C).unsqueeze(0) # just in case
        elif x.dim() == 4:
            x_b = x.reshape(-1, num_nodes, C)          # [1, N, C]
        else:
        ...
        x2 = x1.reshape(x1.size(0), -1)  # [1, num_nodes * dim_feature]
        x2 = self.FC1(x2)
        x2 = self.tanh(x2)
        q = self.FC2(x2)
...
class G_DQN_2(nn.Module):
    def __init__(self,  dim_act, observation_state, args):
        ...
        #DQN
        self.dim_input = ((args.predator2_view_range *2)**2)*self.dim_feature
        self.FC1 = nn.Linear(self.dim_input, 128)
        self.FC2 = nn.Linear(128, dim_act)
