...
class Q_network_MLP(nn.Module):
    ...
    def forward(self, inputs):
        # When 'choose_action', inputs.shape(N,input_dim) # action 뽑는거는 step 마다 하는거니까 이렇게 나오는거고,
        # When 'train', inputs.shape(bach_size,max_episode_len,N,input_dim) #train은 batch training 하니까 이렇게 나오는거고!
        x = F.relu(self.fc2(x))
        Q = self.fc3(x)
        return Q
...
class QMIX_SMAC(object):
    def __init__(self, args):
        ...
        self.eval_parameters = list(self.eval_mix_net.parameters()) + list(self.eval_Q_net.parameters()) # mix_net이랑 q_net 이랑 업데이트 해야하니까 parameter 한번에 붙이고
        if self.use_RMS:
    ...
    def choose_action(self, obs_n, last_onehot_a_n, avail_a_n, epsilon):  # obs_n.shape=(N,obs_dim) last_onehot_a_n.shape=(N,action_dim) avail_a_n.shape=(N,action_dim)
        with torch.no_grad():
            if np.random.uniform() < epsilon:  # epsilon-greedy
                ...
                a_n = [np.random.choice(np.nonzero(avail_a)[0]) for avail_a in avail_a_n] #a_n 에는 각 에이전트 별로 현재 가능한 행동에서의 하나의 행동들을 리스트로 표현한 것!
            else:
                inputs = []
