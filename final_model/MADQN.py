...
class MADQN():  # def __init__(self,  dim_act, observation_state):
    def __init__(self, args, n_predator1, n_predator2, predator1_obs, predator2_obs, dim_act , shared_shape, shared, buffer_size, device = 'cpu'):
        ...
        self.n_prey = args.n_prey  # N of prey
        self.dim_feature = args.dim_feature  # feature channels
        self.epsilon = self.args.eps
        self.eps_decay = self.args.eps_decay
        self.map_size = args.map_size
        self.gamma = args.gamma
        self.book_decay = args.book_decay
        self.eps_min = args.eps_min
        self.replay_times = args.replay_times

        self.jitter_std = args.jitter_std
        ...
        # model
        self.gdqns = [G_DQN(self.dim_act, self.predator1_obs, self.eps_decay).to(self.device)
                         for _ in range(self.n_predator1)
                     ] + [
                         G_DQN(self.dim_act, self.predator2_obs, self.eps_decay).to(self.device)
                         for _ in range(self.n_predator2)
                     ]

        self.gdqn_targets = [G_DQN(self.dim_act, self.predator1_obs, self.eps_decay).to(self.device)
                                for _ in range(self.n_predator1)
                            ] + [
                                G_DQN(self.dim_act, self.predator2_obs, self.eps_decay).to(self.device)
                                for _ in range(self.n_predator2)
        ...
        self.predator1_adj = self.king_adj(self.predator1_view_range * 2)
        self.predator2_adj = self.king_adj(self.predator2_view_range * 2)

        self.predator1_adj = torch.tensor(self.predator1_adj).float()
        self.predator2_adj = torch.tensor(self.predator2_adj).float()
    ...
    def overlap_area(self,a, b, x, y, side_length1, side_length2):
        overlap_width = min(a + side_length1 / 2, x + side_length2 / 2) - max(a - side_length1 / 2,
                                                                              x - side_length2 / 2)
        overlap_height = min(b + side_length1 / 2, y + side_length2 / 2) - max(b - side_length1 / 2,
                                                                               y - side_length2 / 2)
        ...
        # result matrix 의
    def calculate_Overlap_ratio(self, pos):

        centers = pos
        centers = [(x + 1, y + 1) for x, y in centers]

        side_lengths = [self.predator1_view_range * 2 for i in range(self.n_predator1)] + [self.predator2_view_range * 2 for i in
                                                                                 range(self.n_predator2)]

        overlap_matrix = np.zeros((self.n_predator1 + self.n_predator2, self.n_predator1 + self.n_predator2))
        square_areas = np.zeros(self.n_predator1 + self.n_predator2)  # 각 정사각형의 넓이를 저장할 배열
        ...
        # 겹치는 영역의 넓이 계산
        for i in range(self.n_predator1 + self.n_predator2):  # 행의 기준이 되는 에이전트를 의미함
            for j in range(i + 1, self.n_predator1 + self.n_predator2):
                a, b = centers[i]
                x, y = centers[j]
                overlap = self.overlap_area(a, b, x, y, side_lengths[i], side_lengths[j])
    ...
    def calculate_Overlap_ratio_intake(self, past, now):
        ...
        # 겹치는 영역의 넓이 계산
        for i in range(self.n_predator1 + self.n_predator2):  # 행의 기준이 되는 에이전트를 의미함
            for j in range(self.n_predator1 + self.n_predator2):
                a, b = centers_now[i]
                x, y = centers_past[j]
                overlap = self.overlap_area(a, b, x, y, side_lengths[i], side_lengths[j])
                overlap_matrix[i, j] = overlap
    ...
    def prey_number(self, observation_temp):
        prey_pos = np.argwhere(observation_temp[:, :, self.dim_feature - 1] == 1)

        return len(prey_pos)

    # prey 에 있는 number 의 set 을 만든다.
    def prey_num_set(self, prey_deque):
    ...
    def find_prey_positions(self, prey_num_set, prey_deque):
    ...
    def extract_values_based_on_positions(self, prey_set, positions_dict, l2_before, l2_deque):
    ...
    def extrack_plot_outtake(self, prey_deque, shared_mean):
    ...
    def from_semi_shared(self, semi_shared, pos, view_range):
        x_start = pos[1] + self.predator1_view_range - 2
        y_start = pos[0] + self.predator1_view_range - 2

        x_range = int(view_range)
        y_range = int(view_range)

        semi_shared = semi_shared[
            x_start - (x_range - 1):x_start + (x_range + 1), y_start - (y_range - 1): y_start + (y_range + 1)]

        return semi_shared

    def to_semi_shared(self, semi_shared, pos, view_range):
        ...
        semi_shared[
            x_start - (x_range - 1):x_start + (x_range + 1), y_start - (y_range - 1): y_start + (y_range + 1)] += 1

        return semi_shared

    def overlap_tiles(self, observation_temp):
        prey_pos = np.argwhere(observation_temp[:, :] >= 2)

        return prey_pos

    # predator1 이 다른 predator1 과 predator2와 만나는 타일의 위치를 뽑는 함수
    def coor_list_pred1(self, self_pos, pred1_positions, pred2_positions):  # 첫번째는 본인이고,그 뒤로 n_predator1 -1 개의 predator1, n_predator2개의 predator2

        # 주인공 predator1 과 다른 predator1 들이 만나는 타일의 위치
        semi_shared_shape = (self.map_size + (self.predator1_view_range - 2) * 2,  # padding for edges
                             self.map_size + (self.predator1_view_range - 2) * 2)
        semi_shared = np.zeros(semi_shared_shape) # predator 1

        for pos in pred1_positions:  # predator 1 position
            if np.array_equal(pos, self_pos):
                continue
            semi_shared = self.to_semi_shared(semi_shared, pos, self.predator1_view_range)

        with_pred1 = self.from_semi_shared(semi_shared, self_pos, self.predator1_view_range)  # 주인공 predator1가 보는 것을 가져온다.
        ...
        for pos in pred2_positions:  # 처음 n_predator2 개의 predator2에 대해 semi_shared 에 +1을 하여 기록한다.
            semi_shared = self.to_semi_shared(semi_shared, pos, self.predator2_view_range)

        with_pred2 = self.from_semi_shared(semi_shared, self_pos, self.predator1_view_range)  # 주인공 predator1가 보는 것을 가져온다.
    ...
    # predator1 이 다른 predator1 과 predator2와 만나는 타일의 위치를 뽑는 함수
    def coor_list_pred2(self, self_pos, pred1_positions, pred2_positions):  # 첫번째는 predator2 본인이고,그 뒤로 n_predator1 개의 predator1, n_predator2 - 1 개의 predator2

        # 주인공 predator2 과 다른 predator1 들이 만나는 타일의 위치
        semi_shared_shape = (self.map_size + (self.predator1_view_range - 2) * 2,
                             self.map_size + (self.predator1_view_range - 2) * 2)
        semi_shared = np.zeros(semi_shared_shape)
        ...
        with_pred1 = self.from_semi_shared(semi_shared, self_pos, self.predator2_view_range)  # 주인공 predator2가 보는 것을 가져온다.
        overlap_tiles_pred1 = np.argwhere(with_pred1[:, :] >= 1)  # overlap_tiles 에 현재 주인공 predator2와 다른 predator1가 한번이라도 겹친 부분의 위치값들이 기록되어있다.
        ...
        with_pred2 = self.from_semi_shared(semi_shared, self_pos, self.predator2_view_range)  # 주인공 predator2가 보는 것을 가져온다.
        overlap_tiles_pred2 = np.argwhere(with_pred2[:, :] >= 1)  # overlap_tiles 에 현재 주인공 predator2와 다른 predator2가 한번이라도 겹친 부분의 위치값들이 기록되어있다.

        return overlap_tiles_pred1, overlap_tiles_pred2

    def intake_sum(self, book, after_gnn, overlap_tiles):
    ...
    def intake_inner(self, book, after_gnn, overlap_tiles):
    ...
    def dist(self, obs):
        # prey position
        prey_pos = np.argwhere(obs[:,:,self.dim_feature-1] == 1)
    ...
    def from_guestbook(self):
        x_start = self.pos[1] + self.predator1_view_range -2
        y_start = self.pos[0] + self.predator1_view_range -2
